    ...
    if (command !== undefined) {
      if (command.indexOf("A-") == 0) {
        ...
        command = command.replace(
          "A-",
          config.poundCommands.join("-").concat("-")
        );
...
function setupWebSocketServer() {
  ...
  webSocketServer.of("/command").on("connection", function(socket) {
    socket.on("execute", function(command) {
      ICM.executeCommand(command);
    });
  });

  webSocketServer.of("/display").on("connection", function(socket) {
...
function setupIcmEventHandlers() {
  ...
  ICM.events.on("displayChanged", function(displayText) {
    webSocketServer.of("/display").emit("updated", { text: displayText });
  });
